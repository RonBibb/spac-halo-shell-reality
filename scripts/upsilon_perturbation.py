#!/usr/bin/env python3
"""
upsilon_perturbation.py
=======================

Mass-to-light ratio (M*/L) systematic perturbation test for shell_reality (Paper 2, §3.3).

For each galaxy in the v7.0 sample, perturb Upsilon_disk and Upsilon_bulge within
their published systematic uncertainty (~0.1 dex), refit the v7.0 framework
(Burkert + n shells, BIC-selected), and record the perturbed shell selections
and parameters.

If shell selections and parameters are stable across realistic Y perturbations,
shells are not artifacts of baryonic mismodeling.

This script imports run_canonical_fits.py (the v7.0 production fitter) but does
NOT modify it. The v7.0 file can live wherever you keep it — set V7_FITTER_DIR
below if auto-detection doesn't find it.

Usage
-----
  python3 upsilon_perturbation.py [N_REALIZATIONS] [SEED]
Default: N_REALIZATIONS = 20, SEED = 20260510

Outputs (incremental):
  outputs/upsilon_perturbation_per_galaxy.csv
  outputs/upsilon_perturbation_log.txt

Resume: re-running picks up where you left off.
"""
...
# Set False to use the v7.0 production fitter (run_canonical_fits.py).
# Set True to use the reference fitter (for harness testing only — does not
# reproduce v7.0 results).
USING_REFERENCE_IMPLEMENTATION = False

# Where to find run_canonical_fits.py. Leave as None to auto-detect, or set
# explicitly to the absolute path of the directory containing the file.
# Examples:
#   V7_FITTER_DIR = "/Users/ronbibb/Library/CloudStorage/OneDrive-Personal(2)/Documents/Academic"
#   V7_FITTER_DIR = None  # auto-detect from common locations
V7_FITTER_DIR = "/Users/ronbibb/Library/CloudStorage/OneDrive-Personal(2)/Documents/Academic/Localized Residual Structure in SPARC Rotation Curves/sparc-halo-shells/scripts"
UPSILON_DISK_NOMINAL  = 0.5
UPSILON_BULGE_NOMINAL = 0.7
UPSILON_LOG_SIGMA     = 0.10   # 0.1 dex 1-sigma systematic on log10(Upsilon)


def resolve_paths():
    """Auto-detect package layout vs flat working directory."""
...
def find_v7_fitter_dir():
    """Locate the directory containing run_canonical_fits.py.
    Returns Path or raises FileNotFoundError with guidance."""
    ...
    here = Path(__file__).resolve().parent
    pkg_root = here.parent             # shell_reality_v2/
    academic = pkg_root.parent          # parent of shell_reality_v2/
    candidates = [
        pkg_root,                       # alongside this package
        academic,                       # parent dir (typical layout)
        pkg_root / "scripts",           # inside scripts/
        Path.cwd(),                     # cwd
    ]
    for cand in candidates:
        if (cand / "run_canonical_fits.py").exists():
            return cand
    raise FileNotFoundError(
        "run_canonical_fits.py not found in any auto-detected location. "
        "Set V7_FITTER_DIR near the top of upsilon_perturbation.py to the "
        "absolute path of the directory containing run_canonical_fits.py.\n"
        f"Searched: {[str(c) for c in candidates]}"
    )


# ============================================================================
# v7.0 PRODUCTION FITTER WRAPPER
# ============================================================================

def _run_v7(galaxy, rotmod_path, upsilon_disk, upsilon_bulge):
    """Call run_canonical_fits.fit_fw_n_shells() with perturbed Upsilons,
    BIC-select over n in {0, 1, 2}, return harness dict."""
    # Locate and import run_canonical_fits (cached after first call per worker)
    ...
    # Load rotmod (mirrors v7.0's fit_one_galaxy data prep)
    d = np.loadtxt(rotmod_path, comments='#')
    r_all, vobs_all, evobs_all = d[:, 0], d[:, 1], d[:, 2]
    vgas, vdisk, vbul = d[:, 3], d[:, 4], d[:, 5]

    # Compute V_bar^2 with PERTURBED Upsilons (the perturbation injection)
    V2_bar_all = (vgas * np.abs(vgas)
                  + upsilon_disk  * vdisk * np.abs(vdisk)
                  + upsilon_bulge * vbul  * np.abs(vbul))

    # v7.0 masking: keep points where V_obs^2 > V_bar^2
    mask = vobs_all**2 > V2_bar_all
    n_used = int(mask.sum())
    if n_used < 5:
    ...
    # Run v7.0's three fits (Burkert / FW1 / FW2). NFW is skipped.
    ...
    # BIC selection (v7.0 uses k=2/5/8 for n=0/1/2)
    branches = []
    if p_burk is not None:
        branches.append((0, p_burk, chi2_n0, 2))
    if p_fw1 is not None:
        branches.append((1, p_fw1, chi2_n1, 5))
    if p_fw2 is not None:
        branches.append((2, p_fw2, chi2_n2, 8))
...
def _run_reference(galaxy, rotmod_path, upsilon_disk, upsilon_bulge):
    """Simplified Burkert + Gaussian-shell fitter for testing the I/O harness.
    Does NOT faithfully reproduce v7.0. Use only when
    USING_REFERENCE_IMPLEMENTATION = True."""
    ...
    def shell_v2(rr, r_sh, M_sh, sig):
        M_enc = 0.5 * M_sh * (1 + erf_v((rr - r_sh) / (np.sqrt(2) * sig)))
            ...
            try:
                ...
                if res.fun < best[3]:
                    ...
                    for i in range(n_shells):
                        ...
                        sig = (10**res.x[4+3*i]) * r_sh
                        if sig/r_sh > SIGMA_OVER_R_MAX: valid = False
                        shells.append((r_sh, M_sh, sig))
...
def run_v7_framework(galaxy, rotmod_path, upsilon_disk, upsilon_bulge):
    """Dispatch to v7.0 production fitter or reference fitter based on flag."""
    if USING_REFERENCE_IMPLEMENTATION:
        return _run_reference(galaxy, rotmod_path, upsilon_disk, upsilon_bulge)
    return _run_v7(galaxy, rotmod_path, upsilon_disk, upsilon_bulge)


# ============================================================================
# HARNESS — perturbation, parallelism, incremental I/O, resume support
# ============================================================================

CSV_COLUMNS = [
    'Galaxy', 'realization', 'Upsilon_disk', 'Upsilon_bulge',
...
def fit_one_task(args):
    """Worker: fit one (galaxy, realization). Returns row dict for CSV."""
    galaxy, rea_idx, U_disk, U_bulge, rotmod_dir = args
    rotmod_path = str(Path(rotmod_dir) / f"{galaxy}_rotmod.dat")
    np.random.seed((rea_idx * 100003 + abs(hash(galaxy))) % (2**31))

    try:
        result = run_v7_framework(galaxy, rotmod_path, U_disk, U_bulge)
...
def append_row(csv_path, row, write_header):
    """Append one row; flush+fsync so it survives interruption."""
...
def already_done(csv_path):
    """Set of (galaxy, realization) pairs already in the CSV."""
...
def banner_lines():
    if USING_REFERENCE_IMPLEMENTATION:
        return [
            ...
            "*** Results will NOT match canonical CSV. For harness testing  ***",
            "*** only. Set USING_REFERENCE_IMPLEMENTATION = False           ***",
            "*** for real Paper 2 results.                                  ***",
            "******************************************************************",
        ]
    return [
        "==================================================================",
        "=== USING v7.0 PRODUCTION FITTER (run_canonical_fits.py)        ===",
        "=== Perturbed runs use the same fitter that produced canonical. ===",
        "==================================================================",
    ]
...
def main():
    ...
    csv_path = output_dir / 'upsilon_perturbation_per_galaxy.csv'
    log_path = output_dir / 'upsilon_perturbation_log.txt'

    log(log_path, "=== Upsilon perturbation test starting ===")
    for ln in banner_lines():
        log(log_path, ln)

    # Verify v7.0 fitter is locatable before kicking off workers
    ...
    log(log_path, f"  Y_disk_nominal    = {UPSILON_DISK_NOMINAL}")
    log(log_path, f"  Y_bulge_nominal   = {UPSILON_BULGE_NOMINAL}")
    log(log_path, f"  Y_log_sigma       = {UPSILON_LOG_SIGMA} dex")
    ...
    realization_upsilons = []
    for _ in range(n_realizations):
        log_dY_disk  = rng.normal(0, UPSILON_LOG_SIGMA)
        log_dY_bulge = rng.normal(0, UPSILON_LOG_SIGMA)
        U_disk  = UPSILON_DISK_NOMINAL  * 10**log_dY_disk
        U_bulge = UPSILON_BULGE_NOMINAL * 10**log_dY_bulge
        realization_upsilons.append((U_disk, U_bulge))

    log(log_path, "  Realization Y values:")
    for i, (Ud, Ub) in enumerate(realization_upsilons):
        log(log_path, f"    [{i:02d}]  Y_disk = {Ud:.4f}   Y_bulge = {Ub:.4f}")

    all_tasks = []
    for rea_idx, (U_disk, U_bulge) in enumerate(realization_upsilons):
        for galaxy in galaxies:
            if (galaxy, rea_idx) in done:
                continue
            all_tasks.append((galaxy, rea_idx, U_disk, U_bulge, str(rotmod_dir)))
