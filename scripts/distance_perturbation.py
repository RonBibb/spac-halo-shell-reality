#!/usr/bin/env python3
"""
distance_perturbation.py
========================

Distance systematic perturbation test for shell_reality (Paper 2, §3.3).

For each galaxy in the v7.0 sample, perturb its distance D within the published
SPARC uncertainty (e_D), refit the v7.0 framework (Burkert + n shells, BIC-
selected), and record the perturbed shell selections and parameters.

When distance D changes by factor f = D'/D:
  - all radii r scale as r → r·f
  - all baryonic V components scale as V → V·sqrt(f)
    (because V² = GM/r, M ∝ flux·D² ∝ f², r ∝ f, so V² ∝ f)
  - V_obs and e_V_obs are distance-invariant
  - Upsilons unchanged (this test isolates distance systematics)

If shell selections survive realistic distance perturbation, shells are not
artifacts of distance uncertainty.

Per-galaxy perturbations: each galaxy uses its own published e_D from
sparc_sample123.csv.

Usage
-----
  python3 distance_perturbation.py [N_REALIZATIONS] [SEED]
Default: N_REALIZATIONS = 20, SEED = 20260510

Outputs (incremental):
  outputs/distance_perturbation_per_galaxy.csv
  outputs/distance_perturbation_log.txt
...
# Where to find run_canonical_fits.py. None = auto-detect.
V7_FITTER_DIR = "./scripts"

# Where to find galaxy_classifications.csv (for D, e_D values). None = auto-detect.
...
# Nominal SPARC mass-to-light ratios (NOT perturbed in this test)
UPSILON_DISK_NOMINAL  = 0.5
UPSILON_BULGE_NOMINAL = 0.7

# Floor on perturbed distance to prevent pathologies (Mpc)
D_MIN_FLOOR = 0.1
...
def find_v7_fitter_dir():
    ...
    for cand in candidates:
        if (cand / "run_canonical_fits.py").exists():
            return cand
    raise FileNotFoundError(
        "run_canonical_fits.py not found. Set V7_FITTER_DIR near the top "
        "of distance_perturbation.py to the absolute path of the directory "
        "containing run_canonical_fits.py.\n"
        f"Searched: {[str(c) for c in candidates]}"
    )


def find_sparc_catalog():
    """Locate galaxy_classifications.csv (for D, e_D)."""
    ...
    pkg_root = here.parent
    academic = pkg_root.parent
    # Try v7.0 fitter directory if it's locatable
    try:
        v7dir = find_v7_fitter_dir()
    except FileNotFoundError:
        v7dir = None
    candidates = [
        pkg_root / "data" / "galaxy_classifications.csv",
        pkg_root / "galaxy_classifications.csv",
        academic / "galaxy_classifications.csv",
        Path.cwd() / "galaxy_classifications.csv",
    ]
    if v7dir is not None:
        candidates.insert(0, v7dir / "galaxy_classifications.csv")
        candidates.insert(0, v7dir.parent / "galaxy_classifications.csv")
    for cand in candidates:
        if cand.exists():
            return cand
    raise FileNotFoundError(
        "sparc_sample123.csv not found. Set SPARC_CATALOG_PATH near the top "
        "of distance_perturbation.py to the absolute path of the file.\n"
        f"Searched: {[str(c) for c in candidates]}"
    )


# ============================================================================
# v7.0 PRODUCTION FITTER WRAPPER (perturbed distance)
# ============================================================================

def _run_v7(galaxy, rotmod_path, D_nominal, distance_factor):
    """Fit one galaxy with perturbed distance. Returns harness dict.
    distance_factor = D' / D_nominal."""
    ...
    sqrt_f = np.sqrt(f)
    r_pert     = r_all * f
    vgas_pert  = vgas  * sqrt_f
    vdisk_pert = vdisk * sqrt_f
    vbul_pert  = vbul  * sqrt_f
    # V_obs and e_V_obs are distance-invariant

    # V_bar^2 with NOMINAL Upsilons (not perturbed in this test)
    V2_bar_all = (vgas_pert  * np.abs(vgas_pert)
                  + UPSILON_DISK_NOMINAL  * vdisk_pert * np.abs(vdisk_pert)
                  + UPSILON_BULGE_NOMINAL * vbul_pert  * np.abs(vbul_pert))

    # v7.0 masking: V_obs^2 > V_bar^2
...
def _run_reference(galaxy, rotmod_path, D_nominal, distance_factor):
    """Simplified Burkert + Gaussian-shell fitter for harness testing."""
    ...
    if not Path(rotmod_path).exists():
        return {'status': 'no_rotmod', 'n_shells': -1}
    d = np.loadtxt(rotmod_path, comments='#')
    r_all = d[:, 0]; V_obs = d[:, 1]; e_V = np.maximum(d[:, 2], 1.0)
    V_gas, V_disk, V_bulge = d[:, 3], d[:, 4], d[:, 5]
    f = distance_factor; sqrt_f = np.sqrt(f)
    r = r_all * f
    V_bar2 = (V_gas*np.abs(V_gas)*f
              + UPSILON_DISK_NOMINAL  * V_disk *np.abs(V_disk) *f
              + UPSILON_BULGE_NOMINAL * V_bulge*np.abs(V_bulge)*f)
...
def run_v7_framework(galaxy, rotmod_path, D_nominal, distance_factor):
    if USING_REFERENCE_IMPLEMENTATION:
        return _run_reference(galaxy, rotmod_path, D_nominal, distance_factor)
    return _run_v7(galaxy, rotmod_path, D_nominal, distance_factor)
...
CSV_COLUMNS = [
    'Galaxy', 'realization',
    'D_nominal_Mpc', 'D_perturbed_Mpc', 'distance_factor', 'e_D_Mpc',
...
def fit_one_task(args):
    galaxy, rea_idx, D_nom, factor, e_D, rotmod_dir = args
    rotmod_path = str(Path(rotmod_dir) / f"{galaxy}_rotmod.dat")
    np.random.seed((rea_idx * 100003 + abs(hash(galaxy))) % (2**31))

    try:
        result = run_v7_framework(galaxy, rotmod_path, D_nom, factor)
...
def banner_lines():
    if USING_REFERENCE_IMPLEMENTATION:
        return [
            ...
            "*** Results will NOT match canonical CSV.                      ***",
            "******************************************************************",
        ]
    return [
        "==================================================================",
        "=== DISTANCE PERTURBATION TEST                                  ===",
        "=== USING v7.0 PRODUCTION FITTER (run_canonical_fits.py)        ===",
        "=== Per-galaxy D draws from N(D, e_D) using SPARC catalog.      ===",
        "==================================================================",
    ]
...
def main():
    ...
    csv_path = output_dir / 'distance_perturbation_per_galaxy.csv'
    log_path = output_dir / 'distance_perturbation_log.txt'

    log(log_path, "=== Distance perturbation test starting ===")
    ...
    # Load D, e_D from SPARC catalog
    sparc = pd.read_csv(sparc_path)[['Galaxy', 'D', 'e_D']].set_index('Galaxy')
    missing = [g for g in galaxies if g not in sparc.index]
    if missing:
        log(log_path, f"WARNING: {len(missing)} galaxies in canonical CSV missing from SPARC catalog: {missing[:5]}{'...' if len(missing)>5 else ''}")
    ...
    # Pre-generate per-(galaxy, realization) distance factors deterministically
    rng = np.random.default_rng(seed)
    log(log_path, "  Generating per-galaxy distance perturbations...")
    perturbations = {}  # (galaxy, rea_idx) -> (D_nom, factor, e_D)
    fractional_uncertainties = []
    for rea_idx in range(n_realizations):
        for galaxy in galaxies:
            if galaxy not in sparc.index:
                continue
            D_nom = float(sparc.loc[galaxy, 'D'])
            e_D   = float(sparc.loc[galaxy, 'e_D'])
            if D_nom <= 0:
                continue
            # Multiplicative log-space perturbation: sigma_log = e_D / (D ln10)
            sigma_log = e_D / (D_nom * np.log(10))
            log_factor = rng.normal(0, sigma_log)
            factor = max(D_MIN_FLOOR / D_nom, 10**log_factor)
            perturbations[(galaxy, rea_idx)] = (D_nom, factor, e_D)
            if rea_idx == 0:
                fractional_uncertainties.append(e_D / D_nom)

    log(log_path, f"  Per-galaxy fractional distance uncertainties (e_D/D):")
    fu = np.array(fractional_uncertainties)
    log(log_path, f"    median: {np.median(fu)*100:.1f}%, mean: {fu.mean()*100:.1f}%, "
                  f"min: {fu.min()*100:.1f}%, max: {fu.max()*100:.1f}%")

    all_tasks = []
    for (galaxy, rea_idx), (D_nom, factor, e_D) in perturbations.items():
        if (galaxy, rea_idx) in done:
            continue
        all_tasks.append((galaxy, rea_idx, D_nom, factor, e_D, str(rotmod_dir)))
