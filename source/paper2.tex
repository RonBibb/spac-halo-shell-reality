...
\begin{abstract}
We investigate whether the localized residual structures identified in the SPARC rotation-curve framework of Paper I represent a statistically organized population or an accumulation of fitting artifacts. Paper I established the framework and identified a galaxy-level morphology gradient in shell-bearing fraction across $T = 2$--9 (Spearman $\rho = -0.83$, $p = 0.010$). The present paper characterizes the per-shell-level organization of the detected shell population and consolidates artifact-channel and confound-control robustness tests.

On the 101-galaxy NGC 6674-excluded sample, the framework selects 51 shell-bearing systems and 67 individual shells. Five of seven \S3.1 organizational tests cross strict Bonferroni correction; three remain primary after Einasto-backbone-attenuation demotion of two: the $M$-$r$ scaling ($M \propto r^{0.76}$, $\rho = +0.64$, $p < 10^{-4}$), the $\sigma$-$r$ scaling ($\sigma \propto r^{1.04}$ under Burkert, $\rho = +0.78$, $p < 10^{-4}$), and the inner-vs-outer mass ordering in two-shell galaxies (14/16, Wilcoxon $p = 10^{-4}$). Mass-feature signatures preserve under Einasto and gNFW backbone substitutions; width-related signatures ($\sigma$-$r$ slope, $\sigma/r$ value) are specifically degenerate with Einasto curvature flexibility and preserve under gNFW.

The galaxy-level morphology gradient and bulge correlation are statistically inseparable from observational quality covariates in SPARC under multivariate adjustment and four matched-quality subsamples (\S3.3.8); we report them as observed sample-level regularities rather than independently-supported morphological effects. The per-shell signatures are internal to the detected shell population and not subject to this confound.

Two destructive null procedures bracket the real-data signal asymmetrically: residual scrambling under-detects, cross-radius velocity permutation over-detects, and neither reproduces the morphology gradient. Shell selections remain stable under perturbations to stellar mass-to-light ratio (95.1\%), distance (94.1\%), and inclination (98.0\% per-galaxy modal match), and persist in an anti-warp clean subsample. A backbone-shift test shows the smooth profile cannot natively absorb the shell structure. Whether the detected structures correspond to localized halo features or to other forms of organized dynamics remains open.
...
Headline \S3 results use the 101-galaxy NGC 6674-excluded sample. The \S3.3.4 inclination perturbation, \S3.3.5 anti-warp filter, \S3.3.6 backbone-family comparison, \S3.3.7 backbone-shift test, and \S3.3.8 covariate control are run on the 101-galaxy sample directly. The \S3.3.2 ($\Upsilon$) and \S3.3.3 (distance) perturbation suites quote their underlying production batches (102-galaxy $\times$ 20 realizations) for reproducibility; their 101-galaxy re-aggregated statistics differ from the quoted values by less than 0.05 percentage points ($\Upsilon$: 95.10\% $\rightarrow$ 95.05\% per-galaxy modal match; $D$: 94.12\% $\rightarrow$ 94.06\%). The \S3.2 spatial-coherence null tests are reported on the 101-galaxy convention by post-processing the per-galaxy realization data (\texttt{data/shell\_reality\_out\_n100/per\_galaxy.csv}) with NGC 6674 excluded; the underlying 100-realization production batch was run on the 102-galaxy Paper I-aligned sample, and qualitative direction is preserved exactly (empirical $p$-values identical at $2/100$ and $0/100$ on both conventions). Detailed comparison is in \texttt{data/ngc6674\_exclusion\_summary.txt}. NGC 6674 contributes less than 1\% of the \S3.2 realizations and parallel 101-galaxy aggregation of \S3.2 is left as a future computational consistency check; no \S3.2 conclusion depends on NGC 6674's presence or absence at the level of qualitative direction.

\textbf{The anti-warp clean subsample} (23 shells across 23 galaxies on the 101-galaxy sample) restricts to shells satisfying four warp-protection criteria simultaneously: $r_{\rm shell}/R_{\rm HI} < 0.3$, disk-dominated baryonic regime at the shell radius, SPARC quality flag $Q = 1$, and inner shell of any two-shell pair. \S3.3.5 uses this subsample to test whether headline patterns survive in conditions where HI-warp artifacts are most disfavored.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_1_1_bulge_correlation.pdf}
...
\tablecaption{Population $\sigma/r$ distribution across radius quartiles in the 67-shell NGC 6674-excluded sample. The non-monotonic middle quartiles and the small within-galaxy paired effect ($p = 0.30$) motivate the demotion of the $\sigma/r$ quartile gradient to a soft observation in \S3.1.5.\label{tab:auto_1}}
\tablehead{
...
The gap between the population-level pattern and the within-galaxy null is best understood as a population-mixing effect: single-shell galaxies, which contribute 30 of the 51 shell-bearing galaxies and tend to have intrinsically higher $\sigma/r$ (BIC having folded what might have been multi-component structure into one broader Gaussian), populate the inner radial range more densely than the outer range and thereby pull the innermost quartile median upward without producing a corresponding within-galaxy gradient. We treat this as a soft empirical observation rather than a strongly established gradient, and report it for completeness alongside the more robust scaling and inner-vs-outer findings of \S3.1.2 and \S3.1.4.

\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_1_3_sigma_over_r_quartile.pdf}

\caption{\textbf{[Figure 3.1.3 --- Geometric gradient]} $\sigma/r$ vs $r$ scatter (log-x) with quartile boundaries marked as vertical lines and per-quartile median $\sigma/r$ overlaid as horizontal bars. Reference line at $\sigma/r = 0.4$ (framework cap) and at population median 0.275. \emph{Data source:} \texttt{data/antiwarp\_per\_shell.csv} (columns: r\_sh\_kpc, sigma\_over\_r).}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_1_4_inner_vs_outer.pdf}
\caption{\textbf{[Figure 3.1.4 --- Inner-vs-outer in two-shell galaxies]} Paired comparison of inner versus outer shell properties in the 16 two-shell galaxies of the NGC 6674-excluded canonical sample. Outer shells are systematically more massive (14/16; Wilcoxon $p = 10^{-4}$) and broader (14/16; $p = 0.0008$) than inner shells, with median outer-to-inner mass ratio 2.06 and median outer-to-inner radius ratio 2.62. The inner and outer populations occupy statistically distinct $r/r_{\rm vir}$ distributions (KS $D = 0.69$, $p = 0.0007$). \emph{Data source:} \texttt{data/antiwarp\_per\_shell.csv} filtered to two-shell galaxies (16 pairs, 32 shells).}
\end{figure}
...
\textbf{Secondary signatures} (Benjamini-Hochberg FDR-pass at $\alpha = 0.05$; either Bonferroni-fail, entangled with another test, or attenuated under one backbone-family control --- see multi-comparisons summary below for the demotion logic):

\begin{itemize}
  \item \textbf{Bulge correlation:} OR $= 3.67$, Fisher $p = 0.0095$ (one-sided; Bonferroni-fail at $\alpha = 0.05/7 = 0.0071$). Additionally entanglement-demoted by \S3.1.1's bulge--morphology contrast. \emph{The bulge correlation and the morphology gradient (next item) are not statistically independent: they are two projections of the same underlying early-bulged-vs-late-bulgeless contrast (\S3.1.1). They are reported separately for completeness, not as independent lines of evidence; the four secondary signatures listed here therefore represent three statistically independent observations.}
...
\textbf{Multiple-comparisons correction.} All seven \S3.1 tests survive Benjamini-Hochberg FDR correction at $\alpha = 0.05$ (BH-adjusted $p$: $\le 10^{-4}$ for both scaling Spearmans; $0.0002$ for $M$ paired; $0.0011$ for KS and $\sigma$ paired; $0.011$ for bulge; $0.028$ for morphology). Five cross strict Bonferroni ($p < 0.05/7 = 0.0071$): the two scaling tests, both inner-vs-outer Wilcoxons, and the KS. Two of these five are demoted to secondary by Einasto-backbone attenuation --- the $\sigma$ inner-vs-outer Wilcoxon and the KS $r/r_{\rm vir}$ separation (both preserve under gNFW, so the sensitivity is specific to Einasto curvature flexibility; \S3.3.6). The bulge correlation (raw $p = 0.0095$, BH-adjusted $p = 0.011$) does not cross strict Bonferroni and is additionally entanglement-demoted by \S3.1.1. The independent, backbone-control-invariant, non-entangled signatures are therefore the $M$-$r$ scaling, the $\sigma$-$r$ scaling, and the inner-vs-outer mass ordering. The four secondary signatures rely on \S3.2 spatial-coherence nulls and \S3.3.6 backbone-family controls for confirmatory weight rather than on \S3.1 in isolation.
...
Across 100 scramble realizations on the 101-galaxy sample (10,100 fits after NGC 6674 post-processing exclusion), the null distribution of $\rho_{\rm per\,T}$ has mean $-0.258$ with standard deviation $0.214$, range $[-0.786, +0.238]$. The real-data baseline of $-0.762$ falls $2.4\sigma$ below the null mean (Gaussian-equivalent) and at the 2nd percentile of the empirical distribution: 2 of 100 realizations produce a $\rho_{\rm per\,T}$ as negative as real data, giving an empirical one-sided $p_{\rm emp} = 0.02$. The per-galaxy correlation $\rho_{\rm per\,galaxy}$ has null mean $-0.164 \pm 0.057$, range $[-0.313, -0.041]$, with the real-data baseline of $-0.287$ at $2.2\sigma$ below the null mean and an empirical $p_{\rm emp} = 2/100 = 0.02$. The empirical-$p$ estimate is the more reliable significance metric here: the scramble null distribution is visibly non-Gaussian (Figure 3.2.1), with a heavy left tail that the Gaussian-equivalent $z$-score under-resolves.

The scramble null does retain a weakly negative $\rho_{\rm per\,T}$ on average ($-0.258 \pm 0.214$). This is interpretable: the scramble preserves each galaxy's smooth Burkert backbone, and Burkert backbone parameters correlate with morphological type (early-type galaxies have more concentrated halos, deeper baryonic potentials, and more complex residual structure for the framework to find by chance). A correlation in this range is what residual \emph{amplitude variations} alone --- without spatial coherence --- can produce by chance. The real-data correlation of $-0.762$ sits at the 2nd percentile of the scramble null distribution: residual amplitude variations alone can reproduce a correlation this negative in only ~2\% of realizations, while the typical scramble realization produces a correlation roughly one third the real-data magnitude. The morphology gradient depends on the \emph{spatial coherence} of localized residual structure, not merely its amplitude distribution.
...
Across 100 permutation realizations on the 101-galaxy sample (10,100 fits after NGC 6674 post-processing exclusion), the null distribution of $\rho_{\rm per\,T}$ has mean $+0.352$ with standard deviation $0.232$, range $[-0.095, +0.862]$. \textbf{The sign of the null correlation is opposite the real-data correlation.} The real-data baseline of $-0.762$ falls $4.8\sigma$ below the null mean. The empirical one-sided $p_{\rm emp} = 0/100 < 0.01$ (no permute realization reaches the real-data value). The per-galaxy correlation has null mean $+0.164 \pm 0.065$, range $[-0.006, +0.346]$ --- also positive --- with the real-data baseline of $-0.287$ at $6.9\sigma$ below the null mean and again $p_{\rm emp} = 0/100$.
...
\tablecaption{Shell-bearing fractions by T-type bin in the real 102-galaxy sample compared with the medians of the scramble and permute null suites (100 realizations each). Real data shows a monotonic gradient from $T=2$ to $T=9$; scramble nulls flatten the gradient and over-detect at late T; permute nulls reverse the gradient direction.\label{tab:auto_2}}
\tablehead{
...
\begin{itemize}
  \item $\rho_{\rm per\,T}$ rejected at $2.4\sigma$ under scramble (empirical $p = 2/100 = 0.02$), $4.8\sigma$ under permutation (empirical $p = 0/100 < 0.01$)
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_2_1_scramble_null.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_2_2_permute_null.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_1_disk_dynamical_scales.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_3_distance_perturbation.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_4_inclination_perturbation.pdf}
...
These cuts are deliberately conservative; each independently is well-motivated, and the conjunction selects shells where multiple lines of warp protection are active. Of the 67 shells in the canonical 101-galaxy sample (NGC 6674 excluded; \S2.3), 23 (34\%) survive all four cuts. Considered independently, the cuts pass 48, 61, 46, and 51 shells respectively; the combined intersection is 23.

The 23-shell anti-warp clean subsample exhibits most of the same headline patterns as the full population, with one quantitative exception noted below. Table 3.3.5 reports the comparison.

\begin{deluxetable*}{lcc}
\tablecaption{Anti-warp clean subsample comparison on the 101-galaxy NGC 6674-excluded sample. Per-shell scaling signatures and bulge correlation preserve under the cut; the morphology gradient is significant on the full sample ($p = 0.028$) and attenuates below significance on the quality-restricted clean subsample ($p = 0.071$), consistent with the covariate-control result reported in \S3.3.8. Bulge correlation OR computed at the galaxy level on the 23 bulge-dominated and 78 bulgeless galaxies of the 101-galaxy sample (see \S3.1.1).\label{tab:auto_3}}
\tablehead{
\colhead{Quantity} & \colhead{Full sample (n = 67)} & \colhead{Clean subsample (n = 23)} \\
}
\startdata
$M$-$r$ slope (log $M$ vs log $r$) & 0.76 & 1.48 \\
$M$-$r$ Spearman $\rho$ & $+0.64$ ($p < 10^{-4}$) & $+0.66$ ($p < 10^{-3}$) \\
$\sigma$-$r$ slope (log $\sigma$ vs log $r$) & 1.04 & 0.67 \\
$\sigma$-$r$ Spearman $\rho$ & $+0.78$ ($p < 10^{-4}$) & $+0.62$ ($p = 0.002$) \\
$\sigma/r$ population median & 0.28 & 0.32 \\
$\sigma/r$ population std & 0.12 & 0.12 \\
Bulge correlation OR & 3.67 & 1.70 \\
Morphology gradient $\rho_{\rm per\text{-}T}$ & $-0.76$ ($p = 0.028$) & $-0.67$ ($p = 0.071$) \\
\enddata
\end{deluxetable*}

The four per-shell signatures of \S3.1.2--\S3.1.4 --- $M$-$r$ scaling, $\sigma$-$r$ scaling, $\sigma/r$ universality, and inner-vs-outer dimorphism --- preserve under the cut: the $M$-$r$ and $\sigma$-$r$ Spearman coefficients are essentially unchanged in magnitude despite the 66\% reduction in sample size. Effect \emph{directions} are preserved exactly on every quantity. The galaxy-level signatures attenuate in effect size: the bulge correlation odds ratio drops from 3.67 to 1.70 (remains above unity), and the morphology gradient crosses below the $p < 0.05$ threshold on the cleaned subsample ($\rho_{\rm per\,T} = -0.67$, $p = 0.071$, $n = 8$ T-bins). This attenuation is internally consistent with the multivariate covariate test of \S3.3.8: the four anti-warp cuts select for higher-quality shells (high SPARC quality flag, disk-dominated, inner-shell, low-radius), and \S3.3.8 finds that data-quality covariates absorb the apparent T-type effect at the galaxy level. The clean-subsample result is therefore the predicted empirical signature of a quality-confounded galaxy-level gradient: restricting to high-quality shells reduces the within-sample T-quality collinearity that was inflating the bivariate gradient on the full sample.

A structural observation accompanies the survival result. The clean subsample's $M$-$r$ slope steepens from 0.76 to 1.48 and the $\sigma$-$r$ slope flattens from 1.04 to 0.67, while $\sigma/r$ median rises from 0.28 to 0.32. The fourth cut deliberately retains only inner-of-pair and single shells, dropping the outer of every two-shell system. These shifts therefore reflect the inner-shell sub-population's distinct structural signature, the per-shell-position dimorphism reported elsewhere in \S3.1, rather than a residual cleaning artifact. The shifts are diagnostic of structure, not of warps.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_5_antiwarp_clean.pdf}

\caption{\textbf{[Figure 3.3.5 --- Anti-warp clean subsample]} Two-panel comparison: $\sigma$-vs-$r$ scatter (left) and $M$-vs-$r$ scatter (right). Full-sample shells (n=67) in light gray; anti-warp clean shells (n=23) highlighted. Linear fits overlaid for each population with slopes annotated (full vs clean: $M$-$r$ 0.76$\rightarrow$1.48, $\sigma$-$r$ 1.04$\rightarrow$0.67). \emph{Data source:} \texttt{data/antiwarp\_per\_shell.csv} (use is\_clean flag for the clean subsample).}
\end{figure}
...
Table 3.3.6 reports the per-shell signatures under all three backbones --- the present paper's net-new content over Paper I.

\begin{deluxetable*}{lccc}
\tablecaption{Per-shell organizational signatures under three smooth-halo backbones (Burkert, Einasto, gNFW) on the 101-galaxy NGC 6674-excluded sample. Mass-feature signatures ($M$-$r$ scaling and inner-vs-outer mass ordering) preserve under all three backbones; width-related signatures ($\sigma$-$r$ slope and $\sigma/r$ distribution) attenuate under Einasto specifically and preserve under gNFW, indicating that the $\sigma$-related findings are degenerate with Einasto curvature flexibility rather than generic to backbone-family choice.\label{tab:auto_4}}
\tablehead{
...
Bulge correlation OR & 3.67 ($p = 0.0095$) & 4.32 ($p = 0.003$) & 4.22 ($p = 0.003$) \\
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_7_backbone_shift.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig_3_3_8_covariate_forest.pdf}
\caption{\textbf{[Figure 3.3.8 --- Covariate forest plot]} Coefficient summaries from the multivariate covariate test. \emph{Left panel (a):} Logistic regression coefficients (z-standardized continuous predictors, binary bulge flag) for galaxy-level shell-bearing classification on the full 101-galaxy NGC 6674-excluded sample. Horizontal bars show 95\% confidence intervals; significant predictors ($p < 0.05$) in red, non-significant in gray. The three significant predictors ($\log N_{\rm RC}$, $\log \langle V_{\rm err} \rangle$, $\log L_{3.6}$) are all observational quality covariates; T-type and bulge dominance are non-significant after adjustment. \emph{Right panel (b):} T-type coefficient across the full sample and four matched-quality subsamples ($A$ = top half $N_{\rm pts}$; $B$ = bottom half $\langle V_{\rm err} \rangle$; $C$ = $A \cap B$; $D$ = top half $N_{\rm pts}$ within each T-bin; $n = 53, 51, 24, 56$). T-type remains non-significant in all five samples, with three of the four matched subsamples showing weakly positive coefficients --- the opposite sign from the bivariate $\rho_{\rm per\,T} = -0.83$ gradient. \emph{Data sources:} \texttt{data/covariate\_results.csv} (panel a) and \texttt{data/covariate\_matched\_results.csv} (panel b).}
\end{figure}

\subsubsection{Random-structure null tests (cross-reference to \S3.2)}

The scrambling and permutation null tests of \S3.2 establish that the shell selections are statistically distinguishable from random structure: cross-radius $V_{\rm obs}$ permutation produces morphology gradients of opposite sign in 0/100 realizations, and within-galaxy residual scrambling reaches the real-data $\rho_{\rm per\,T}$ in only 2/100 realizations (empirical $p = 0.02$; the typical scramble realization sits at one third the real-data magnitude). For cross-reference, the standardized null deviations on the corresponding $z$-scale are $-2.4$ and $-4.8$ for $\rho_{\rm per\,T}$ under scramble and permute respectively, and $-2.2$ and $-6.9$ for $\rho_{\rm per\,galaxy}$ (with the empirical $p$-floor at $1/N = 0.01$). Both tests target the hypothesis that the BIC-selected shells could be detector-flexibility artifacts of fitting random or shape-randomized data. The asymmetric failure modes (scramble under-detects, permute over-detects and reverses gradient sign) further establish that the shell population is a property of spatially coherent localized residual structure, not a property that survives destruction of either the residual coherence (scramble) or the rotation-curve shape (permute).

\subsubsection{Combined verdict}

The localized structures detected by the framework satisfy eight complementary tests against artifact origin, with an additional scope-of-claim caveat (item 9) on which galaxy-level statistical signatures the framework can and cannot claim independently of observational quality covariates:
...
\begin{enumerate}
  ...
  \item Shell selections survive inclination perturbations drawn from per-galaxy SPARC uncertainties (realized $|i' - i|$ median 1.77$^{\circ}$, max 29.1$^{\circ}$) with 95.9\% per-fit and 98.0\% per-galaxy mode-match.
  ...
  \item (Scope-of-claim caveat) Galaxy-level shell-bearing classification is predicted by observational quality covariates in multivariate logistic regression and remains so in four matched-quality subsamples designed to reduce T-quality collinearity (\S3.3.8); the galaxy-level morphology gradient and bulge correlation reported in \S3.1.1 and \S3.1.5 are therefore reported as observed sample-level regularities statistically inseparable from quality covariates in SPARC, not as independently-supported morphological effects. The per-shell organizational signatures (\S3.1.2--\S3.1.4) are internal to the detected shell population and therefore not directly subject to this galaxy-level confound; their affirmative support against artifact origin is carried by tests 1--8 above.
\end{enumerate}

The eight artifact-channel tests address complementary mechanisms: disk dynamics, baryonic input systematics ($\Upsilon$, $D$, $i$), warp geometry, backbone-family flexibility, backbone-shell coupling, and random structure. The signal survives all eight in concert. We therefore conclude that the localized structures detected by the framework are not consistent with artifacts of the fitting procedure or measurement systematics within the channels tested. Independently, the galaxy-level statistical signatures reported in \S3.1.1 and \S3.1.5 are confounded with observational quality covariates and are reported as such (item 9); this confound is structural to the SPARC catalog rather than a property of the analysis pipeline.
...
The principal limitations of the present work are four. The analysis depends on the Paper I decomposition framework and does not establish decomposition uniqueness; the \S3.3.6 and \S3.3.7 backbone controls demonstrate robustness within one strict increase in backbone flexibility and against the decoupled-basis-function alternative, but broader-family comparisons (generalized NFW with free inner slope, spline or nonparametric profiles, hierarchical Bayesian smooth-halo models) remain necessary. The analysis examines one-dimensional rotation-curve structure rather than full two-dimensional velocity fields; explicit modeling of noncircular motions, bars, spiral structure, and warped geometries would provide a stronger separation between localized halo structure and baryonic dynamical effects. The shell population is modest in size, particularly within the two-shell subsample and the anti-warp clean sample, limiting the statistical power of some higher-order comparisons. Finally, the galaxy-level morphology gradient and bulge correlation reported in Paper I and \S3.1.1 are statistically inseparable from observational quality covariates in the SPARC catalog (\S3.3.8): the intrinsic SPARC correlations between T-type and data quality are sufficiently severe ($\rho(T, \log L_{3.6}) = -0.81$, $p = 10^{-24}$) that the catalog does not contain enough counter-examples to disentangle morphology from quality at the available sample size. We report these galaxy-level signatures as observed sample-level regularities; an independent kinematic or external-tracer confirmation channel (e.g., HI velocity dispersion at shell radii via THINGS) would be required to establish a clean morphological effect.
...
Future work should therefore focus on four directions, with the first carrying the highest priority because it is orthogonal to the \S3.3.8 quality confound and could rescue the galaxy-level morphology gradient as a clean morphological effect: (i) resolved \ion{H}{1} velocity-dispersion measurements at predicted shell radii from interferometric cubes (e.g., THINGS, \citealt{Walter2008}; LITTLE THINGS methodology in \citealt{Oh2011}), predicting elevated line widths at shell radii relative to interleaved non-shell radii in the same galaxy and providing a kinematic-population test independent of rotation-curve fit-quality covariates; (ii) testing the persistence of the shell population under broader halo-profile families and nonparametric decompositions; (iii) constructing hierarchical population-level statistical models for shell occurrence and scaling behavior; and (iv) evaluating whether existing dynamical or cosmological simulations naturally reproduce the observed organizational patterns.

Beyond rotation-curve-based and kinematic-population tests, independent observational channels include strong-lensing substructure measurements, stellar-stream architectures in the Milky Way and nearby galaxies, and dwarf-galaxy active galactic nucleus surveys. The patterns reported here generate testable predictions in each of these channels; their resolution does not depend on decisions about which smooth-halo decomposition is adopted.
...
Analysis and figure generation used the scientific Python stack: NumPy (Harris et al. 2020), SciPy (Virtanen et al. 2020), pandas (McKinney 2010), and Matplotlib (Hunter 2007). The complete analysis pipeline --- production scripts, intermediate data, and figure-generation code --- is publicly available at \url{https://github.com/RonBibb/sparc-halo-shell-reality} and archived at Zenodo (\dataset[DOI: 10.5281/zenodo.20356863]{https://doi.org/10.5281/zenodo.20356863}).
