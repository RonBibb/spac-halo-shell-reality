...
\caption{\textbf{[Figure 3.3.8 --- Covariate forest plot]} Coefficient summaries from the multivariate covariate test. \emph{Left panel (a):} Logistic regression coefficients (z-standardized continuous predictors, binary bulge flag) for galaxy-level shell-bearing classification on the full 102-galaxy sample. Horizontal bars show 95\% confidence intervals; significant predictors ($p < 0.05$) in red, non-significant in gray. The three significant predictors ($\log N_{\rm RC}$, $\log \langle V_{\rm err} \rangle$, $\log L_{3.6}$) are all observational quality covariates; T-type and bulge dominance are non-significant after adjustment. \emph{Right panel (b):} T-type coefficient across the full sample and four matched-quality subsamples ($A$ = top half $N_{\rm pts}$; $B$ = bottom half $\langle V_{\rm err} \rangle$; $C$ = $A \cap B$; $D$ = top half $N_{\rm pts}$ within each T-bin). T-type remains non-significant in all five samples, with three of the four matched subsamples showing weakly positive coefficients --- the opposite sign from the bivariate $\rho_{\rm per\,T} = -0.83$ gradient. \emph{Data sources:} \texttt{data/covariate\_results.csv} (panel a) and \texttt{data/covariate\_matched\_results.csv} (panel b).}
\end{figure}
