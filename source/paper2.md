...
7. The Burkert backbone systematically deforms when shells are removed ($\rho_0$ rises by a factor of $\approx 4.2$, scale radius $a$ contracts by a factor of $\approx 2.1$, in 88.5% of shell-bearing galaxies); the non-shell-bearing control population does not show this systematic deformation (§3.3.7). Shells are dynamically coupled to the backbone, not decoupled basis functions.
8. Shell-selection statistics are inconsistent with random or shape-randomized data at the $3\sigma$–$8\sigma$ level (§3.2).
