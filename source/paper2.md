...
Exceptions to this exclusion are §3.2 (spatial-coherence null tests), §3.3.2–3.3.4 (mass-to-light, distance, and inclination perturbation tests), and §3.3.7 (backbone-shift test), which retain the Paper I-aligned 102-galaxy sample for computational consistency with their production runs. NGC 6674 contributes less than 1% of the realizations or fits in each of these analyses, and parallel 101-galaxy analyses of §3.1 show no qualitative change to any reported conclusion. Full reruns under the unified 101-galaxy convention are deferred to future work.
...
**Mass-feature signatures (preserved or strengthened under Einasto):** The mass-radius scaling preserves direction and slightly steepens (0.76 → 0.82) with stronger Spearman $\rho$ ($+0.64 \to +0.69$). The bulge correlation strengthens (OR rises from 3.67 to 4.32; $p$ drops from $\approx 0.01$ to 0.003). The per-shell-binned morphology gradient strengthens slightly ($\rho_{\rm per\text{-}T}$ moves from $-0.76$ to $-0.87$). The inner-vs-outer mass ordering survives at high significance (14/19 with $p < 10^{-3}$, comparable to the Burkert 14/16 result).

**Width-related signatures (attenuated under Einasto):** The width-radius slope drops substantially (1.04 → 0.38), with the Spearman $\rho$ falling from $+0.78$ to $+0.38$. The $\sigma/r$ population median shifts from 0.275 to 0.173. The inner-vs-outer width ordering, robust under Burkert ($p = 0.0008$), reduces to marginal significance under Einasto ($p = 0.10$).

This pattern admits a natural interpretation. Galaxy-level shell-bearing classifications (Paper I §3.7) and localized-mass-feature properties (this section) reflect mass concentrations whose detection does not depend strongly on the smooth backbone's specific functional form. Shell-*width* parameters, by contrast, are partially degenerate with backbone profile shape: Einasto's free $\alpha$ parameter can absorb part of the radial variation that, under Burkert, is parameterized by shell width $\sigma$. The shifts in $\sigma$-$r$ slope and $\sigma/r$ median are therefore consistent with shell-width parameters partially decomposing differently under a more flexible backbone, while the integrated mass-feature signal remains robust to backbone choice.
...
We test this directly. For each galaxy in the canonical sample, we refit the framework at each of $n_{\rm shells} \in \{0, 1, 2\}$ and record the resulting Burkert backbone parameters $(\rho_0, a)$ at every level. We then compare $(\rho_0, a)$ at the BIC-selected $n$ against $(\rho_0, a)$ at the constrained $n = 0$, separately for shell-bearing and non-shell-bearing galaxies. The test uses the 102-galaxy Paper I-aligned sample (NGC 6674 included for compatibility with the production run; this analysis joins §3.2 and §3.3.2-3.3.4 in the §2.3 partition-asymmetry disclosure).

**Result for the 52 shell-bearing galaxies.** When the framework is allowed to select shells, the Burkert backbone systematically relaxes:
...
- **46 of 52 (88.5%) shell-bearing galaxies show the predicted "absorbing pattern" of decreasing $\rho_0$ accompanied by increasing $a$ when shells are allowed.** Five (9.6%) show the opposite pattern; one (1.9%) shows mixed signs. Under a null of random sign per galaxy ($p = 0.5$), an exact two-sided binomial test rejects the random-sign null at $p \approx 1.0 \times 10^{-8}$ for 46/52, tightening to $p \approx 2.3 \times 10^{-9}$ when the single ambiguous-sign galaxy is dropped (46/51). The directional claim — that the smooth backbone shifts to absorb shell structure when shells are removed — is falsified by chance with vanishing probability under this test.
...
**Interpretation.** The backbone-shift test addresses the strong form of the basis-function alternative — that shells are decoupled localized basis functions absorbing residuals the smooth profile could otherwise leave unchanged. They are not decoupled: when shells are removed, the smooth Burkert profile must systematically deform (denser core, more compact scale) to compensate for the lost flexibility. Combined with the §3.3.6 Einasto comparison (organizational signatures persist under a more flexible smooth-halo family), this places the basis-function alternative in a difficult position. The Einasto test addresses *passive* basis-function flexibility (residual organization survives backbone-family change); the backbone-shift test addresses *decoupled* basis-function flexibility (removing shells forces active backbone deformation). Together they bracket the alternative from two complementary directions.

> **[Figure 3.3.7 — Backbone shift]** Two-panel figure: (left) $\log_{10}(\rho_0[\text{BIC}]/\rho_0[n=0])$ distribution for shell-bearing (n=52) and non-shell-bearing (n=50) galaxies as overlaid histograms; (right) $\log_{10}(a[\text{BIC}]/a[n=0])$ distribution for same. Vertical lines at the medians; null line at zero shift.
> *Data source:* `data/backbone_shift.csv` produced by `scripts/backbone_shift_test.py`.
> *Status:* Placeholder; generation script pending.
