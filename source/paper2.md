...
We investigate whether the localized residual structures identified in the SPARC rotation-curve framework of Paper I represent a statistically organized population or an accumulation of fitting artifacts. Paper I established the framework, validated it against synthetic null tests and Einasto-backbone substitution, and demonstrated a galaxy-level morphology gradient in shell-bearing fraction across $T = 2$–9 (per-galaxy permutation $p = 0.002$, per-bin Spearman $\rho = -0.83$, $p = 0.010$). The present paper carries out a complementary, consolidated treatment of artifact-channel robustness for the shell population as a whole, and characterizes the per-shell-level organization of the population identified in Paper I.

Using the canonical 102-galaxy Paper I sample with NGC 6674 excluded for the 101-galaxy primary analyses (following Paper I §4.4), the framework selects 51 shell-bearing systems via Bayesian Information Criterion, with shell-bearing fraction decreasing monotonically toward later morphological type (Spearman $\rho = -0.762$, $p = 0.028$ — the NGC 6674-excluded version of Paper I's $\rho = -0.833$, $p = 0.010$ result).

The 67 fitted shells exhibit several forms of internal organization at the per-shell level (net-new content over Paper I). Shell mass scales approximately as $M \propto r^{0.76}$ ($\rho = +0.64$, $p < 10^{-4}$) and shell width scales near-linearly with radius ($\sigma \propto r^{1.04}$ under the Burkert backbone; $\rho = +0.78$, $p < 10^{-4}$), implying a characteristic fractional width $\sigma/r \approx 0.27$ across more than two decades in radius. The width-related signatures (and the implied $\sigma/r$ near-universality) are partially backbone-baseline-dependent — under the Einasto backbone reported in Paper I §3.7, the $\sigma$-$r$ slope and $\sigma/r$ median shift substantially while mass-feature signatures are preserved (per-shell analysis in §3.3.6). Within two-shell galaxies, outer shells are systematically more massive than inner shells ($p < 10^{-3}$). Bulge-dominated galaxies are preferentially shell-bearing relative to bulgeless systems (odds ratio $= 3.67$, Fisher $p = 0.0064$); because bulge-dominated systems are concentrated at early types, this signature is statistically near-equivalent to the morphology gradient and the two should be treated as projections of a single underlying contrast rather than as independent lines of evidence (§3.1.1).

We test whether these patterns can arise from random or coherence-destroyed structure using two destructive null procedures applied directly to the observed rotation curves — complementing the synthetic-mock nulls reported in Paper I §3.5. Residual scrambling suppresses the morphology gradient, while cross-radius velocity permutation over-produces shell detections, yielding opposite failure modes relative to the real data. Shell selections additionally remain stable under perturbations to stellar mass-to-light ratio, galaxy distance, and inclination, and the principal scaling relations persist in a conservatively defined anti-warp clean subsample. A backbone-shift test shows that the smooth profile cannot natively absorb the structure without measurable parameter shift, disfavoring an account in which shells behave as decoupled basis functions.

These results disfavor random fitting behavior, baryonic measurement systematics, outer-disk warp artifacts, decoupled basis-function flexibility, and (in combination with Paper I §3.7 and §4.4) backbone-family flexibility and per-galaxy stellar mass-to-light variation as dominant explanations for the shell population. Whether the detected structures correspond to localized halo features or to other forms of organized galactic dynamics remains an open question.
...
This pattern indicates that inner and outer shells within two-shell galaxies occupy statistically distinct $r/r_{\rm vir}$ distributions under the Burkert backbone, consistent with multi-scale population stratification. Two caveats temper the claim. First, the present sample of 16 paired galaxies is modest. Second, the separation does not survive Einasto-backbone substitution: under Einasto the two distributions become essentially indistinguishable (KS $D = 0.11$, $p \approx 1.0$ on the Einasto two-shell sample; see §3.3.6). We therefore classify this finding in §3.1.5 alongside the inner-vs-outer width ordering as a Burkert-baseline-dependent signature.
...
- **Bulge correlation** (statistically near-equivalent to the morphology gradient in this sample; see §3.1.1): OR $= 3.67$, Fisher $p = 0.0064$. Although the raw $p$-value formally satisfies the Bonferroni threshold for seven tests ($p < 0.0071$), this signature does not provide independent confirmatory weight: the §3.1.1 analysis demonstrates that the bulge correlation and the morphology gradient are two statistical projections of a single underlying contrast between bulge-rich early-type and bulgeless mid-to-late-type galaxies, and the morphology gradient itself does not cross Bonferroni. We therefore classify the bulge correlation as a secondary signature alongside the gradient it projects from, rather than as a fifth independent primary signal. Strengthens under Einasto (OR $= 4.32$, $p = 0.003$).
- **Morphology gradient** (galaxy-level): $\rho_{\rm per\text{-}T} = -0.762$ ($p = 0.028$). Sensitive to NGC 6674's exclusion: Paper I reports $\rho = -0.833$, $p = 0.010$ on the 102-galaxy convention, and the shift to $p = 0.028$ on the present 101-galaxy convention reflects that single galaxy's removal alone (rationale in §2.3). Strengthens under Einasto ($\rho = -0.87$, $p = 0.005$). Bonferroni-corrected $p = 0.196$ (single-test interpretation requires the §3.2 spatial-coherence nulls for confirmatory weight; see §3.2.3).
...
- **Two-population separation in $r/r_{\rm vir}$**: KS $D = 0.6875$, $p = 0.0007$ under Burkert. The raw $p$-value crosses the Bonferroni threshold ($p < 0.0071$); the finding is classified secondary rather than primary because it does not survive Einasto-backbone substitution (KS $D = 0.11$, $p \approx 1.0$ under Einasto; §3.3.6). Consistent with internal population stratification under the Burkert backbone, with the backbone-dependent character placing it in the same group as the inner-vs-outer width ordering and the $\sigma$-$r$ slope (§3.1.4, §3.3.6).
...
**Multiple-comparisons correction.** Across the seven primary §3.1 statistical tests (counted as: $\sigma$-$r$ Spearman, $M$-$r$ Spearman, $M$ inner-vs-outer Wilcoxon, $\sigma$ inner-vs-outer Wilcoxon, bulge Fisher, morphology Spearman, KS $r/r_{\rm vir}$), all seven survive Benjamini-Hochberg FDR correction at $\alpha = 0.05$ (BH-adjusted $p$-values: $\le 10^{-4}$ for both scaling Spearman tests, 0.0002 for $M$ paired, 0.0011 for KS, 0.0011 for $\sigma$ paired, 0.0075 for bulge, 0.028 for morphology). Six raw $p$-values cross strict Bonferroni correction ($p < 0.05/7 = 0.0071$): the two scaling tests, the two inner-vs-outer Wilcoxon tests, the bulge Fisher test, and the KS test. Of these six, three are classified as secondary signatures by criteria distinct from the Bonferroni threshold: (i) the bulge correlation is statistically entangled with the morphology gradient (§3.1.1: two projections of a single underlying contrast) and is therefore not an independent additional signal; (ii) the $\sigma$ inner-vs-outer Wilcoxon attenuates from $p = 0.0008$ to $p = 0.10$ under Einasto-backbone substitution (§3.3.6); (iii) the KS $r/r_{\rm vir}$ separation attenuates from $p = 0.0007$ to $p \approx 1.0$ under Einasto (§3.3.6). The effective set of independent, backbone-invariant, non-entangled primary signatures therefore remains three: the $M$-$r$ scaling, the $\sigma$-$r$ scaling under Burkert, and the inner-vs-outer mass ordering. The morphology gradient (which fails Bonferroni at $p = 0.028$) joins the three demoted Bonferroni-passing tests in the secondary tier, and the four secondary signatures rely on the §3.2 spatial-coherence nulls and §3.3.6 backbone-family control for confirmatory weight rather than on the §3.1 tests in isolation.
...
#### 3.3.6 Backbone-family control: Einasto comparison at the per-shell level
...
Paper II extends that result to the **per-shell level**, asking which of the population-organizational signatures of §3.1 — bulge correlation, $M$-$r$ scaling, $\sigma$-$r$ scaling, $\sigma/r$ universality, inner-vs-outer asymmetry — survive Einasto-backbone substitution at the level of individual shell parameters, and which depend on the specific smooth-halo family used as the baseline. The Einasto-backbone fits are those reported in Paper I §3.7 (identical pipeline, identical BIC model selection over $n_{\rm shells} \in \{0, 1, 2\}$, identical strict $\sigma/r \le 0.4$ enforcement); we restrict to the 101-galaxy NGC 6674-excluded sample for consistency with §3.1.

**101-galaxy reproduction of Paper I's classification result.** Under the NGC 6674-excluded sample, classification agreement becomes 89 of 101 (88.1%), with 10 Burkert-shell-bearing galaxies reclassified as non-shell-bearing under Einasto and 2 Einasto-shell-bearing galaxies reclassified as non-shell-bearing under Burkert. This reproduces Paper I §3.7's 90/102 = 88.2% result with the expected one-galaxy shift from excluding NGC 6674, confirming the consistency of the two sample conventions. The 101-galaxy version gives 43 shell-bearing galaxies under Einasto (vs 51 under Burkert), with 62 total shells (vs 67) and 19 two-shell galaxies (vs 16).

Table 3.3.6 reports the per-shell signatures under both backbones — Paper II's net-new content over Paper I.
...
| Bulge correlation OR | 3.67 ($p = 0.0064$) | 4.32 ($p = 0.003$) |
| $M$-$r$ slope | 0.76 | 0.82 |
...
| Two-population separation $r/r_{\rm vir}$ KS | $D = 0.69$ ($p = 7\times 10^{-4}$) | $D = 0.11$ ($p \approx 1.0$) |

The per-shell signatures fall into two groups by behavior under backbone change.

**Mass-feature signatures (preserved or strengthened under Einasto):** The mass-radius scaling preserves direction and slightly steepens (0.76 → 0.82) with stronger Spearman $\rho$ ($+0.64 \to +0.69$). The bulge correlation strengthens (OR rises from 3.67 to 4.32; $p$ drops from 0.0064 to 0.003). The per-shell-binned morphology gradient strengthens slightly ($\rho_{\rm per\text{-}T}$ moves from $-0.76$ to $-0.87$). The inner-vs-outer mass ordering survives at high significance (14/19 with $p < 10^{-3}$, comparable to the Burkert 14/16 result).

**Width-related and radial-position signatures (attenuated under Einasto):** The width-radius slope drops substantially (1.04 → 0.38), with the Spearman $\rho$ falling from $+0.78$ to $+0.38$. The $\sigma/r$ population median shifts from 0.275 to 0.173. The inner-vs-outer width ordering, robust under Burkert ($p = 0.0008$), reduces to marginal significance under Einasto ($p = 0.10$). The two-population separation in $r/r_{\rm vir}$, robust under Burkert ($D = 0.69$, $p < 10^{-3}$), vanishes under Einasto ($D = 0.11$, $p \approx 1.0$); under the more flexible backbone the inner and outer shell populations occupy essentially indistinguishable normalized-radius distributions (medians 0.050 vs 0.052), and the median radial positions of the populations shift outward relative to the Burkert configuration (Burkert medians 0.012 / 0.027 vs Einasto 0.050 / 0.052).

This pattern admits a natural interpretation. Galaxy-level shell-bearing classifications (Paper I §3.7) and localized-mass-feature properties (this section) reflect mass concentrations whose detection does not depend strongly on the smooth backbone's specific functional form. Shell *width* and *radial position* parameters, by contrast, are partially degenerate with backbone profile shape: Einasto's free $\alpha$ parameter can absorb part of the radial variation that, under Burkert, is parameterized by shell width $\sigma$ and shell position $r$. The shifts in $\sigma$-$r$ slope, $\sigma/r$ median, and the loss of $r/r_{\rm vir}$ population separation are therefore consistent with shell-position and shell-width parameters partially decomposing differently under a more flexible backbone — shells under Einasto end up at larger normalized radii and with less pairwise radial separation in two-shell systems — while the integrated mass-feature signal remains robust to backbone choice.

The principal conclusion of §3.1 — that the shell population exhibits reproducible internal organization beyond what the smooth backbone alone explains — survives the Einasto control analysis in the load-bearing morphology, bulge, and mass-scaling signatures. The width-related signatures should be understood as defined relative to a specific choice of smooth-halo family. The backbone-family caveat therefore applies more strongly to width-related signatures than to mass-feature signatures; the latter are demonstrated here to survive at least one strict increase in backbone flexibility, complementing Paper I's galaxy-level result with per-shell-level evidence.

> **[Figure 3.3.6 — Per-shell Einasto comparison]** Two-panel figure: (left) per-shell scaling relations $M$-$r$ (top) and $\sigma$-$r$ (bottom) for shells fitted under both backbones, with linear fits annotated showing slope preservation for $M$-$r$ and slope attenuation for $\sigma$-$r$; (right) $\sigma/r$ distribution histograms under both backbones showing the population-median shift from 0.275 to 0.173.
> *Data source:* Paper I `data/einasto_full_sample_results.csv` joined with canonical Burkert per-shell fits.
> *Status:* Placeholder; generation script pending.
