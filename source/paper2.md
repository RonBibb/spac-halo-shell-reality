...
This pattern indicates that inner and outer shells within two-shell galaxies occupy statistically distinct $r/r_{\rm vir}$ distributions under the Burkert backbone, consistent with multi-scale population stratification. Two caveats temper the claim. First, the present sample of 16 paired galaxies is modest. Second, the separation behaves asymmetrically under two backbone-family controls: under the Einasto backbone the two distributions become essentially indistinguishable (KS $D = 0.11$, $p \approx 1.0$ on the Einasto two-shell sample), while under the gNFW backbone the separation preserves and slightly strengthens ($D = 0.89$, $p < 10^{-3}$ on the 9 gNFW two-shell galaxies; see §3.3.6). The attenuation is therefore specific to Einasto-$\alpha$-curvature flexibility rather than a generic property of more-flexible backbone families. We classify this finding in §3.1.5 in the secondary tier on the basis of the Einasto behavior, but flag that the more precise characterization is Einasto-$\alpha$-specifically sensitive.
...
- **Inner-vs-outer width ordering**: 14/16 outer broader than inner ($p = 0.0008$) under Burkert. Backbone behavior is asymmetric: attenuates to 13/19, $p = 0.10$ under Einasto, but strengthens to 9/9, $p = 0.002$ under gNFW (§3.3.6). Reported as suggestive of the same physical asymmetry as the mass ordering with Einasto-$\alpha$-specific sensitivity rather than generic backbone-family sensitivity.
- **Two-population separation in $r/r_{\rm vir}$**: KS $D = 0.6875$, $p = 0.0007$ under Burkert. The raw $p$-value crosses the Bonferroni threshold ($p < 0.0071$); the finding is classified secondary rather than primary on the basis of asymmetric backbone behavior. Under Einasto the separation effectively vanishes ($D = 0.11$, $p \approx 1.0$); under gNFW it strengthens further ($D = 0.89$, $p = 7 \times 10^{-4}$; §3.3.6). The attenuation is Einasto-$\alpha$-specific rather than a generic backbone-family effect — gNFW (a different but equally flexible backbone family) preserves the signature.
...
This pattern admits a structural interpretation. Einasto's free $\alpha$ parameter modifies the smooth-halo curvature at all radii, including the intermediate radii (a few to tens of kpc) where SPARC shells live; the backbone can therefore partially absorb radial structure into curvature changes, smearing the inner-vs-outer separation seen under Burkert and shifting fitted shells outward (median Einasto $r/r_{\rm vir}$ 0.050 vs Burkert 0.012 inner / 0.027 outer). gNFW's free $\gamma$ parameter, by contrast, primarily controls the inner-slope behavior at $r \ll r_s$ and has only weak influence on the backbone at the intermediate radii where shells reside. The same shell-width and shell-position parameters that decompose differently under Einasto retain their Burkert-like character under gNFW. Mass-feature signatures (mass scaling, bulge correlation, inner-vs-outer mass ordering) survive both controls because integrated mass within a localized feature is not degenerate with either kind of backbone-shape parameter at the relevant radii. The width-related and radial-position signatures, by contrast, are degenerate specifically with Einasto-style curvature flexibility — a more precise statement than the Burkert-baseline-dependence inferred from the Einasto control alone.

A side observation supports the structural interpretation. Under the gNFW backbone, $\gamma$ pegs at the lower boundary $\gamma = 0$ for 60 of 101 galaxies (59.4%); the median $\gamma$ across the sample is 0.0 and the interquartile range is $[0.00, 0.53]$. This is the well-known SPARC core preference recovered in a backbone-family that can freely choose between core and cusp. For the majority of the sample, the gNFW backbone is functioning as a cored profile (with a different functional form than Burkert), which explains why the shell signatures under gNFW so closely track those under Burkert: gNFW with $\gamma = 0$ is effectively a third cored backbone family alongside Burkert, while Einasto with its $\alpha$-curvature parameter operates in a structurally different regime.

The principal conclusion of §3.1 — that the shell population exhibits reproducible internal organization beyond what the smooth backbone alone explains — survives both backbone-family controls in the load-bearing morphology, bulge, and mass-scaling signatures. The width-related and radial-position signatures are now characterized more precisely: they are Einasto-$\alpha$-specifically sensitive rather than generically backbone-family-sensitive, since gNFW (a different but equally flexible backbone family) preserves them. The backbone-family caveat therefore applies more narrowly than the Einasto-only control suggested. The mass-feature signatures are demonstrated here to survive at least two strict increases in backbone flexibility, complementing Paper I's galaxy-level result with per-shell-level evidence under two distinct backbone-family controls.
