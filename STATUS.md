...
- ~~gNFW backbone test as natural follow-on to Einasto (§3.3.6) and backbone-shift (§3.3.7)~~ — **DONE 2026-05-21.** gNFW analysis added to §3.3.6 alongside Einasto. Result: gNFW absorbs more shells than Einasto (39 vs 43 shell-bearing) but the Einasto-attenuated signatures (σ-r slope, σ/r median, σ inner-vs-outer, KS r/r_vir) all preserve or strengthen under gNFW. Attenuation is therefore Einasto-α-specific rather than generic backbone-family-sensitive. Producer: `scripts/run_gnfw_fits.py`; output: `data/gnfw_full_sample_fits.csv`.
...
- **gNFW backbone-family control added to §3.3.6.** Producer script `scripts/run_gnfw_fits.py` parallel-fits the 102-galaxy SPARC T=2-9 sample under gNFW backbone ($\rho \propto r^{-\gamma}(1+r/r_s)^{\gamma-3}$, $\gamma \in [0,2]$ free; mass enclosed via cumulative-trapezoid integration; identical BIC selection and shell caps to canonical pipeline). Output: `data/gnfw_full_sample_fits.csv` (102 rows × 46 columns), `data/run_gnfw_fits.log`. Full-sweep wall time 6.3 min with 18 workers on M1 Ultra. Diagnostic results: (1) shell-bearing fraction 39/101 = 38.6% (vs Burkert 50.5%, Einasto 42.6%); 9 two-shell galaxies (vs 16, 19); all 9 gNFW two-shell ⊂ 16 Burkert two-shell. (2) γ distribution: 59% peg at γ=0 lower boundary, median γ=0, recovering the classic SPARC core preference. (3) Mass-feature signatures preserved or strengthened under both Einasto and gNFW: M-r slope 0.76→0.82→0.97; bulge OR 3.67→4.32→4.22; inner-vs-outer M 14/16→14/19→8/9. (4) Width-related and radial-position signatures attenuate under Einasto but PRESERVE or STRENGTHEN under gNFW: σ-r slope 1.04→0.38 vs 1.04→0.86; σ/r median 0.275→0.173 vs 0.275→0.230; inner-vs-outer σ 14/16→13/19 (p=0.10) vs 14/16→9/9 (p=0.002); KS D 0.69→0.11 vs 0.69→0.89 (p=7×10⁻⁴). Conclusion: the Einasto attenuation is α-curvature-specific, not generic backbone-family-sensitive. Updates: §3.3.6 header renamed to "Einasto and gNFW comparisons"; intro paragraph expanded; new gNFW methodology paragraph; classification reproduction paragraph updated to include gNFW (83/101 = 82.2%); Table 3.3.6 expanded to 4 columns (Burkert/Einasto/gNFW); two-groups discussion rewritten; mass-feature paragraph extended; width-and-position paragraph rewritten to acknowledge gNFW preservation; interpretive paragraph rewritten distinguishing α-curvature vs γ-inner-slope flexibility; new γ-distribution observation paragraph added; closing paragraph generalized to both controls; Figure 3.3.6 caption expanded. §3.1.4 KS caveat and §3.1.5 σ Wilcoxon / KS secondary entries updated to characterize attenuation as Einasto-α-specific rather than generic. Multi-comparisons paragraph updated. Tier classifications unchanged (Option 1: conservative reframing).
