...
1. **Resolve remaining Tier 2 discrepancies** between manuscript-reported and recomputed values:
   - KS D-statistic for r/r_vir (manuscript 0.47 vs recompute 0.688 — likely r_vir source difference)
   - Bulge OR Fisher p (manuscript "≈ 0.01" — pick a specific value from {scipy default 0.0064, Boschloo/Barnard/mid-p 0.0038})
   - Wilcoxon p-values §3.1.4 and σ outer>inner count are **resolved** in current draft (recompute values adopted).
